...
<div class="page-cont w-100 h-100 p-3 d-flex flex-column gap-3">
  ...
  <div>
    <div class="d-flex align-items-center mb-2">
      <p class=" fw-bold h5">Permintaan Partisipasi Tim Sebaya</p>
              ...
              <tr>
                ...
                <td>
                  <?= $this->render("/component/_btn_opt", [
                    ...
                    'look' => ['detail', 'case_id' => $member->certification->id],
                  ]); ?>
                </td>
              </tr>
            <?php endforeach ?>
            <?php if (empty($peer_team_member_request)): ?>
              <tr><td colspan="8" class="text-center text-muted py-3">Tidak ada permintaan baru.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
  ...
  <div>
    <div class="d-flex align-items-center mb-2">
      ...
      <a href="#collapse-running" class=" text-decoration-none text-black fw-bolder h6 ms-2" data-bs-toggle="collapse" role="button" aria-expanded="true" aria-controls="collapse-running">
        <i class="fa-solid fa-chevron-down"></i>
      </a>
    </div>
    <div class="bg-white px-2 py-4 rounded-2 shadow border-1 border">
          ...
          <tbody>
            <?php foreach ($peer_team_member_uncompleted as $key => $member) : ?>
              <tr>
                ...
                <td>
                  <?php if ($member->certification->status === CertificationStatus::PEER_REVIEW) : ?>
                    <a href="<?php echo Url::to(['peer-review', 'certification_id' => $member->certification->id]) ?>" class="s-btn-main btn btn-sm"><i class="fa-solid fa-edit"></i></a>
                  <?php else:  ?>
                    <a href="<?php echo Url::to(['detail', 'case_id' => $member->certification->id]) ?>" class="s-btn-main btn btn-sm"><i class="fa-solid fa-magnifying-glass"></i></a>
                  <?php endif  ?>
                </td>
              </tr>
            <?php endforeach ?>
            <?php if (empty($peer_team_member_uncompleted)): ?>
              <tr><td colspan="7" class="text-center text-muted py-3">Tidak ada sertifikasi berjalan.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
  ...
  <div>
    <div class="d-flex align-items-center mb-2">
      ...
      <a href="#collapse-history" class=" text-decoration-none text-black fw-bolder h6 ms-2" data-bs-toggle="collapse" role="button" aria-expanded="true" aria-controls="collapse-history">
        <i class="fa-solid fa-chevron-down"></i>
      </a>
    </div>
    <div class="bg-white px-2 py-4 rounded-2 shadow border-1 border">
          ...
          <tbody>
            <?php foreach ($peer_team_member_completed as $key => $member) : ?>
              <tr>
                ...
                <td><?= Html::encode($member->certification->grade ? CertificateGrade::list()[$member->certification->grade] : '-') ?></td>
                <td>
                  <a href="<?php echo Url::to(['detail', 'case_id' => $member->certification->id]) ?>" class="s-btn-main btn btn-sm"><i class="fa-solid fa-magnifying-glass"></i></a>
                </td>
              </tr>
            ...
            <?php if (empty($peer_team_member_completed)): ?>
              <tr><td colspan="8" class="text-center text-muted py-3">Belum ada riwayat sertifikasi.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
